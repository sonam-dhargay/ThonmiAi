...
const STORAGE_KEY = 'bod_skyad_why_v4';
  ...
  const [content, setContent] = useState(() => {
    ...
    if (saved) {
      try {
        ...
        console.error("Failed to parse why content", e);
      }
    }
    return {
      title: TIBETAN_STRINGS.whyThonmiTitle,
      content: TIBETAN_STRINGS.whyThonmiContent,
    };
  });
  ...
  return (
    <div className="max-w-4xl mx-auto py-14 px-7 transition-colors duration-300">
      <div className="bg-white dark:bg-stone-800 rounded-[2.5rem] shadow-xl p-10 md:p-14 border border-red-50 dark:border-stone-700 relative transition-colors duration-300">
        ...
        <div className="absolute top-10 right-10 opacity-10 z-0 pointer-events-none">
        ...
        {isEditing ? (
          <input 
            ...
            className="w-full text-3xl font-bold text-slate-900 dark:text-stone-100 mb-8 bg-slate-50 dark:bg-stone-900 border border-red-100 dark:border-stone-700 rounded-2xl p-4 outline-none focus:ring-2 focus:ring-red-400 Tibetan-text relative z-10"
          />
        ) : (
          <h1 className="text-3xl font-bold text-slate-900 dark:text-stone-100 mb-8 leading-tight relative z-10">
            {content.title}
          </h1>
        )}

        <div className="prose prose-xl prose-slate dark:prose-invert max-w-none relative z-10">
          {isEditing ? (
            <textarea 
