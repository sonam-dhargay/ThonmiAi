    ...
    <div className={`flex w-full mb-10 ${isUser ? 'justify-end' : 'justify-start'} animate-fade-in`}>
      <div
        className={`max-w-[85%] md:max-w-[75%] px-7 py-5 rounded-3xl relative group transition-all duration-300 ${
          isUser
            ? 'bg-gradient-to-br from-indigo-600 to-violet-700 text-white rounded-tr-none shadow-xl shadow-indigo-100'
            : 'bg-white text-slate-800 border border-slate-100 rounded-tl-none shadow-md hover:shadow-lg'
        }`}
      >
        {message.imageUrl && (
          <div className="mb-4 overflow-hidden rounded-2xl shadow-inner border border-slate-100">
            <img 
        ...
        <div className={`prose prose-xl ${isUser ? 'prose-invert' : 'prose-indigo'} max-w-none Tibetan-text leading-relaxed`}>
        ...
        {!isUser && (
          <div className="mt-4 pt-3 border-t border-slate-50 text-[11px] text-slate-400 font-medium italic opacity-70 Tibetan-text leading-tight">
            {TIBETAN_STRINGS.aiCaveat}
          </div>
        )}
        
        <div className={`flex items-center justify-between mt-5 pt-4 border-t ${isUser ? 'border-white/10 flex-row-reverse' : 'border-slate-50 flex-row'}`}>
          ...
          {!isUser && (
            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-y-1 group-hover:translate-y-0">
              <button
                onClick={handleCopy}
                className={`p-1.5 rounded-lg transition-all ${copied ? 'text-green-600 bg-green-50' : 'text-slate-400 hover:text-indigo-600 hover:bg-indigo-50'}`}
              ...
              {onSaveToDict && (
                <button
                  ...
                  className="p-1.5 rounded-lg hover:bg-indigo-50 transition-all text-slate-400 hover:text-indigo-600"
              ...
              {onFeedback && (
                <div className="flex items-center gap-0.5 bg-slate-50 rounded-lg p-0.5">
                  <button
                    ...
                    className={`p-1 rounded-md transition-all ${message.feedback === 'up' ? 'text-indigo-600 bg-white shadow-sm' : 'text-slate-300 hover:text-indigo-400'}`}
                  >
                  ...
                  <button
                    ...
                    className={`p-1 rounded-md transition-all ${message.feedback === 'down' ? 'text-red-500 bg-white shadow-sm' : 'text-slate-300 hover:text-red-400'}`}
                  >
