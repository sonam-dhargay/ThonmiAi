...
const DictionaryPanel: React.FC<DictionaryPanelProps> = ({ isOpen, onClose, initialTerm }) => {
  ...
  useEffect(() => {
    if (initialTerm) {
      ...
      // Determine which dict to open based on content
  ...
  const filteredEntries = currentDict
    ...
    .sort((a, b) => {
      // Logic for sorting based on dictionary type
      if (activeDict === 'terminology') {
        // Terminology: English Alphabetical Order
        const termA = a.englishTerm?.toLowerCase() || '';
        const termB = b.englishTerm?.toLowerCase() || '';
        return termA.localeCompare(termB);
      } else {
        // Regular: Tibetan Alphabetical Order (Lexicographical)
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-80 md:w-[30rem] glass-panel shadow-[-15px_0_60px_rgba(0,0,0,0.12)] z-50 flex flex-col border-l border-white/40 animate-slide-in">
      <div className="p-7 border-b border-slate-100/50 flex items-center justify-between">
        <h3 className="text-2xl font-bold text-slate-800 flex items-center gap-3">
           <span className={`w-2.5 h-2.5 rounded-full shadow-lg ${activeDict === 'regular' ? 'bg-indigo-500 shadow-indigo-100' : 'bg-amber-500 shadow-amber-100'}`}></span>
           {activeDict === 'regular' ? TIBETAN_STRINGS.dictionary : TIBETAN_STRINGS.terminologyDict}
        </h3>
        <button onClick={onClose} className="p-2.5 hover:bg-white hover:shadow-md rounded-full transition-all text-slate-400 hover:text-indigo-600">
      ...
      <div className="flex px-5 py-2.5 gap-1.5 bg-slate-100/50 mx-6 mt-4 rounded-2xl border border-slate-200/50 shadow-inner">
        <button
          onClick={() => setActiveDict('regular')}
          className={`flex-1 py-2.5 text-sm font-bold transition-all rounded-xl ${
            activeDict === 'regular' ? 'bg-white shadow-md text-indigo-600' : 'text-slate-400 hover:text-slate-600'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2.5 text-sm font-bold transition-all rounded-xl ${
            activeDict === 'terminology' ? 'bg-white shadow-md text-indigo-600' : 'text-slate-400 hover:text-slate-600'
          }`}
        >
      ...
      <div className="p-7 flex flex-col flex-1 overflow-hidden">
        <div className="relative mb-7">
          <input
            ...
            placeholder={TIBETAN_STRINGS.searchPlaceholder}
            className="w-full pl-14 pr-6 py-4.5 bg-slate-100 focus:bg-white border-2 border-transparent focus:border-indigo-500 rounded-[1.8rem] outline-none transition-all text-xl font-medium shadow-inner"
          />
        ...
        {showAddForm ? (
          <div className="bg-white p-6 rounded-[2rem] border-2 border-indigo-50 mb-8 animate-slide-up shadow-2xl shadow-indigo-50/50 overflow-y-auto max-h-[60vh]">
            <div className="space-y-4">
              {activeDict === 'terminology' && (
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest px-2">{TIBETAN_STRINGS.englishTerm}</label>
                  <input
                    value={newEntry.englishTerm || ''}
                    onChange={(e) => setNewEntry(prev => ({ ...prev, englishTerm: e.target.value }))}
                    placeholder="English Term"
                    className="w-full p-3 bg-slate-50 border border-slate-100 rounded-2xl outline-none text-lg font-bold focus:bg-white focus:border-indigo-500 transition-all"
                  />
                </div>
              )}
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest px-2">{TIBETAN_STRINGS.tibetanTerm}</label>
                <input
                  value={newEntry.term || ''}
                  onChange={(e) => setNewEntry(prev => ({ ...prev, term: e.target.value }))}
                  placeholder="མིང་།"
                  className="w-full p-3 bg-slate-50 border border-slate-100 rounded-2xl outline-none text-2xl font-bold focus:bg-white focus:border-indigo-500 transition-all"
                />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest px-2">འགྲེལ་བཤད། (Definition)</label>
                <textarea
                  value={newEntry.definition || ''}
                  onChange={(e) => setNewEntry(prev => ({ ...prev, definition: e.target.value }))}
                  placeholder="འགྲེལ་བཤད།"
                  rows={4}
                  className="w-full p-3 bg-slate-50 border border-slate-100 rounded-2xl outline-none text-lg focus:bg-white focus:border-indigo-500 transition-all resize-none Tibetan-text"
                />
              </div>
              <div className="flex gap-2 pt-2">
                <button
                  onClick={handleAddTerm}
                  className="flex-1 py-3.5 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-[1.2rem] font-bold shadow-xl shadow-indigo-100 hover:shadow-indigo-200 active:scale-95 transition-all"
                >
                  {TIBETAN_STRINGS.saveToDict}
                </button>
                <button
                  onClick={() => setShowAddForm(false)}
                  className="px-5 py-3.5 bg-slate-100 text-slate-500 rounded-[1.2rem] font-bold hover:bg-slate-200 transition-all"
                >
                  ✖
                </button>
              </div>
          ...
          <button
            ...
            className="w-full py-4 mb-8 bg-white border-2 border-dashed border-indigo-100 text-indigo-300 rounded-[1.8rem] hover:border-indigo-400 hover:text-indigo-600 font-bold transition-all text-sm group"
          >
        ...
        <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
          {filteredEntries.length > 0 ? (
            filteredEntries.map((entry, idx) => (
              <div key={idx} className="p-6 bg-white border border-slate-50 rounded-[2.2rem] shadow-sm group hover:border-indigo-100 hover:shadow-xl transition-all transform hover:-translate-y-1 duration-300">
                <div className="flex flex-col gap-1 mb-3">
                  {entry.englishTerm && (
                    <span className="text-sm font-bold text-indigo-500 tracking-wide uppercase">{entry.englishTerm}</span>
                  )}
                  <h4 className="text-2xl font-bold text-slate-900 leading-tight group-hover:text-indigo-700 transition-colors">{entry.term}</h4>
                </div>
            ...
            <div className="text-center py-20 bg-slate-50/50 rounded-[2.5rem] border border-slate-100">
              ...
              <button 
                ...
                className="mx-auto py-4 px-10 bg-indigo-600 text-white rounded-[1.5rem] hover:bg-indigo-700 transition-all disabled:opacity-50 flex items-center gap-3 shadow-xl shadow-indigo-100 font-bold active:scale-95"
              >
            ...
            <div className="text-center py-24 opacity-30">
               <div className="w-24 h-24 bg-slate-200/40 rounded-full flex items-center justify-center mx-auto mb-8">
