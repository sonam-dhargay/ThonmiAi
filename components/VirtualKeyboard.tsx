...
const VirtualKeyboard: React.FC<VirtualKeyboardProps> = ({ mode, onKeyPress, onBackspace }) => {
  ...
  return (
    <div className="bg-red-50/60 dark:bg-stone-950/60 backdrop-blur-2xl p-3 md:p-4 rounded-[2.5rem] border border-white/50 dark:border-stone-800 mt-3 flex flex-col gap-2 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.15)] dark:shadow-black/50 max-w-full overflow-hidden select-none animate-slide-up transition-colors duration-300">
        ...
        <div key={i} className="flex justify-center gap-1.5 md:gap-2">
          ...
          {row.map((key) => (
            <button
              ...
              className={`min-w-[34px] h-12 md:min-w-[48px] md:h-16 bg-white dark:bg-stone-800 border border-red-50 dark:border-stone-700 rounded-[1.2rem] flex items-center justify-center text-2xl font-medium transition-all shadow-sm active:scale-90 hover:shadow-lg dark:hover:shadow-black/40 hover:border-amber-100 dark:hover:border-amber-900/50 flex-1 max-w-[60px] text-slate-800 dark:text-stone-100`}
            >
              {key}
            </button>
          ))}
          ...
          {((!isEnglish && i === 3) || (isEnglish && i === 2)) && (
            <button
              type="button"
              onClick={onBackspace}
              className="w-14 h-12 md:w-20 md:h-16 bg-red-100/80 dark:bg-red-900/40 hover:bg-orange-600 dark:hover:bg-red-600 hover:text-white border border-red-200/40 dark:border-red-900/20 rounded-[1.2rem] flex items-center justify-center text-red-900 dark:text-red-400 active:scale-95 transition-all shadow-md shrink-0"
            >
      ...
      <div className="flex justify-center gap-3 px-3">
        <button
          type="button"
          onClick={() => onKeyPress(' ')}
          className={`h-12 md:h-16 bg-white/95 dark:bg-stone-800 border border-red-50 dark:border-stone-700 rounded-[1.2rem] flex items-center justify-center text-slate-400 dark:text-stone-500 hover:bg-white dark:hover:bg-stone-700 hover:shadow-xl shadow-md active:scale-[0.98] transition-all font-bold text-xs uppercase tracking-[0.25em] ${isEnglish ? 'w-64 md:w-96' : 'w-48 md:w-64'}`}
        >
