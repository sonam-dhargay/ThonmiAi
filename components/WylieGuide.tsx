      ...
      <div className="bg-white w-full max-w-3xl max-h-[90vh] rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col border border-white/40 animate-slide-up">
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar">
          {sections.map((section, idx) => (
            <div key={idx}>
              <h4 className="text-[11px] font-bold text-slate-400 uppercase tracking-[0.25em] mb-6 flex items-center gap-3">
                <span className="w-2 h-2 rounded-full bg-indigo-500"></span>
                {section.title}
              </h4>
              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-4">
                {section.items.map((item, i) => (
                  <div key={i} className="flex flex-col items-center p-4 bg-slate-50 border border-slate-100 rounded-[1.5rem] hover:border-indigo-200 hover:shadow-lg transition-all group">
                    <span className="text-sm font-mono text-indigo-500 font-bold mb-2 group-hover:scale-110 transition-transform">{item.w}</span>
                    <span className="text-3xl text-slate-800 Tibetan-text">{item.t}</span>
                  </div>
                ))}
              </div>
            </div>
